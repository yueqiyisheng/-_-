...
def createtree(dataset,labels,data_full,labels_full,data_test): # 输入的labels仅是前面feature，没有最后的分类结果label
	...
	# 测试：如果划分子树没有提高准确率，则不进行划分，返回改节点的投票率作为节点类别
	if testing(mytree,data_test,labels_full) < testingMajor(majority(classlist),data_test):  
		return mytree
...
for line in data:
	line = line.strip()
	line1 = line.split(',')[:]
	line2 = [float(item) for item in line1[1:]]
	line2.extend((line1[0]))
	dataset0.append(line2)

dataset = dataset0[:140][:]
data_test = dataset0[140:-1][:]
...
lensestree = sz_c3_tree_yujianzhi.createtree(dataset,lenseslabels,data_full,labels_full,data_test)
